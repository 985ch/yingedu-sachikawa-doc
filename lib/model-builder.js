...
const { generate, sachikawaMysql } = require('egg-sequelize-mg');
...
class ModelBuilder {
  ...
  async genDatabaseModels(buildDoc = false) {
    ...
    await generate(this.app.config.sequelize,
      this.config.tables,
      path.join(this.app.baseDir, 'app'),
      { mysql: sachikawaMysql(docJson) });
    if (buildDoc) {
  ...
  async buildIndexDoc(groupTexts, groups, dirPath) {
    ...
    for (const group of groups) {
      ...
      for (const table of groupInfo) {
        text += `| ${this.getLinkText(table.url, table.table)} | ${table.desc} |\n`;
      }
      text += '\n';
    }
