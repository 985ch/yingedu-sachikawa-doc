...
// 数据模型构造器
class ModelBuilder {
  constructor(app, tables, config, indexName = '索引') {
  ...
  async buildDbDoc(comments, dbJson, config) {
    ...
    for (const table in dbJson) {
      ...
      const { group, desc, url } = await this.buildTableDoc(table, dbJson[table], commentList, dirPath);
  ...
  // 构造单个数据表的文档
  async buildTableDoc(name, table, comments, dirPath) {
    // 构造字段说明
    ...
    for (const comment of comments) {
      if (!table.comment || table.comment === '') {
        this.logger.error(`字段[${name} -> ${comment.columnName}]缺少注释，请立即补充注释`);
      }
    ...
    // 提交数据到showdoc服务器
    const url = await this.saveDoc(dirPath, name, text);
